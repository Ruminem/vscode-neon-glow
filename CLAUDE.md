...
뒤의 것은 윈도우 사용자 설치면 `patchStaged` 가 미리 막음 — 백그라운드 업데이트가 다음 버전을
옆 `<커밋>` 폴더에 몇 시간 먼저 풀어 두고, 재시작은 `Code.exe` 만 바꾸고 옛 폴더를 지울 뿐 새
폴더는 안 건드림(1.139 소스와 inno-updater 로그로 확인, 2026-09-24). 그래서 `activate`·`deactivate`
에서 그 폴더를 패치함. `deactivate` 는 종료 때 약 5초 기다려 주고 패치는 27ms 였음. **체크섬
검사를 빼지 말 것** — 덜 풀린 파일을 패치하면 `.bak` 이 깨진 파일로 떠짐.
...
- **한국어 절은 음슴체로 씀.** 상세는 전역 규칙(`~/.claude/CLAUDE.md`)의 음슴체 절.
...
- **알림·툴팁은 `vscode.l10n.t('영어 원문')`으로 씀.** 한국어는 `l10n/bundle.l10n.ko.json`에
  ...
  - 프리셋 이름(Subtle 등)은 영어로 두고, 설명만 번역함.
...
- `state.json`을 임시 파일에 쓰고 `rename`하는 원자적 쓰기는 해 보고 뺐음. Windows에서는
  읽는 쪽이 파일을 열고 있으면 `rename`이 `EPERM`으로 거절돼(동시 읽기·쓰기 3초에 2318번
  중 2120번) 결국 제자리 쓰기로 떨어짐. 반쯤 쓰인 판독은 렌더러가 버리고 다음 폴링에 읽음.
- DOM으로 닿는 표면은 사실상 끝남. 미니맵·오버뷰 룰러는 캔버스라 CSS가 못 감.
- 터미널은 `terminal.integrated.gpuAcceleration`이 `off`일 때만 span 으로 그려짐. 그때는
  `.xterm-rows span`에 `text-shadow`가 먹음. 나머지 값에서는 캔버스인데, 1.117에서 재 보니
  그 캔버스는 불투명이라 `drop-shadow`가 제 뒤에 가려지고, 불투명 이미지를 밝은 부분만 뽑아
  번지게 할 수 있는 `url(#...)` SVG 필터는 합성된 캔버스에서 조용히 무시됨 — 같은 자리에
  건 `blur()`는 패널 전체를 흐리게 하므로 필터가 안 닿는 것이 아니라 참조 필터만 버려지는 것임.
