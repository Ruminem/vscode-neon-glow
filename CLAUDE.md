...
설정: `glowLayers` `maxBlur` `brightness` `minChroma` `chromaSpan` `floor` `minLightness` `cursorTrail` `saveShake` `findGlow` `selectionGlow` `occurrenceGlow` `gutterGlow` `bracketMatchGlow` `squiggleGlow` `diffGlow` `lineHighlightGlow` `breakpointGlow` `caretArc` `caretArcMinJump` `caretArcDuration` `caretArcOnDrag`
