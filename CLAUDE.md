...
- **설정 설명은 언어별 파일에 씀.** `package.json`에는 `%neonGlow.<설정>.markdownDescription%`
  ...
  - **명령 이름(`contributes.commands`의 `title`)도 `%neonGlow.<명령>.title%` 키로 두고
    번역함.** 한국어 화면의 명령 팔레트는 한국어 이름 아래에 영어 원문을 붙여 주므로 영어로
    찾던 이름도 그대로 찾힘. 앞의 `Neon Glow:`는 번역하지 않고, 낱말은 언어팩을 따름
    (`설정/해제`, `사용 안 함`). README는 영어 이름으로 적음.
