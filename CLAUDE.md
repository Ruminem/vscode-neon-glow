...
설정: `glowLayers` `maxBlur` `brightness` `minChroma` `chromaSpan` `floor` `minLightness` `cursorTrail` `saveShake` `findGlow` `selectionGlow` `occurrenceGlow` `gutterGlow` `bracketMatchGlow` `squiggleGlow` `diffGlow` `lineHighlightGlow` `breakpointGlow` `snippetGlow` `renameGlow` `breathe` `caretArc` `caretArcMinJump` `caretArcDuration` `caretArcOnDrag`
...
- **기능을 더하거나 빼면 문서가 같은 커밋에서 따라감.** 설정이나 명령 하나에 딸린
  ...
  1. 설정 설명 — `package.nls.json`(영어)과 `package.nls.ko.json`(한국어) **둘 다**.
     `package.json`의 스키마에는 키만 둠. 설정 화면에서 읽는 자리라 여기가 제일 길어도 됨
