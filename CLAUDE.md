...
설정: `glowLayers` `maxBlur` `brightness` `minChroma` `chromaSpan` `floor` `minLightness` `cursorTrail` `saveShake` `findGlow` `selectionGlow` `occurrenceGlow` `gutterGlow` `bracketMatchGlow` `caretArc` `caretArcMinJump`
