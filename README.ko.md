...
스위치는 켜져 있는데 빛날 수가 없는 세 상태도 여기서 알려준다. 에디터에서 보면 셋이 구분되지
않기 때문이다. **경고색 배경**은 번들이 패치되지 않았거나, 예전 릴리스의 페이로드를 담고 있거나,
이 창이 뜬 뒤에 패치돼서 렌더러가 아직 부팅 때의 번들을 돌리고 있다는 뜻이다. 어느 쪽인지는
툴팁이 말해주고, 클릭하면 그것을 해결하는 동작이 실행된다. 마지막 검사는 일부러 한쪽으로만
판단한다. "창 다시 로드"는 확장
...
설정(`Ctrl+,`)을 열고 `Neon Glow`를 검색한다. 바꾸면 열려 있는 에디터에 1초 안팎으로 반영된다.
재패치도 재시작도 필요 없다.

| 설정 | 기본값 | |
|------|--------|--|
| `neonGlow.brightness` | `1.0` | 전체 세기. `0`이면 색은 그대로 두고 글로우만 없앤다 |
| `neonGlow.minChroma` | `0.30` | 이보다 밋밋한 색은 빛나지 않는다 — 본문 텍스트를 걸러내는 것이 이 값이다 |
| `neonGlow.chromaSpan` | `0.50` | 임계값 위로 크로마가 얼마나 더 있어야 최대 세기에 닿는가 |
| `neonGlow.floor` | `0.40` | `minChroma`를 겨우 넘긴 색의 세기 |
| `neonGlow.minLightness` | `0.25` | 아무리 채도가 높아도 이보다 어두우면 건너뛴다 |

재시작이 필요 없는 이유는 이 값들이 패치 안에 살지 않기 때문이다. 확장이 토글과 같은
`state.json`에 값을 쓰고, 렌더러가 다음 폴링 때 범위를 다듬어 적용하면서 테마의 토큰 색으로부터
스타일시트를 다시 만든다. 그래서 번들은 한 번만 쓰면 된다.

같은 값이 `neon-glow.js` 맨 위의 기본값이기도 하다. CLI로만 설치한 경우 그쪽이 쓰인다 — 거기엔
값을 보내줄 확장이 없다. 그것을 고쳤다면 `node install.js`를 다시 돌리고 재시작해야 한다.
...
**확장을 업데이트해도 패치는 그대로다.** 페이로드는 `workbench.js`에 살고, 새 VSIX를 설치해도
그 파일은 건드리지 않는다. 그래서 새 릴리스가 렌더러에 추가한 것은 죽은 채로 남는다 — 설정이
생겼는데 아무 일도 안 일어나는 식이다. 주입된 배너가 어느 버전으로 쓰였는지를 함께 담고 있어서,
확장이 불일치를 알아채고 다시 패치할지 물어본다. 수락하면 다른 패치와 마찬가지로 완전 재시작이
필요하다.
