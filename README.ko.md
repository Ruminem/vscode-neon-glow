...
없다. 키 할당은 양쪽 다 그대로 된다. `commandPalette`의 `when` 절은 팔레트에서만 숨길 뿐
키바인딩 시스템에서 숨기지 않는다.

키는 `neonGlow.on`과 `neonGlow.off` 두 개이고, 둘 다 일부러 긍정형이다. `when` 절은 설정되지 않은
키와 false인 키를 구분하지 못한다. 그래서 `enabled` 키 하나만 쓰면 리로드부터 확장이 활성화되기
전까지의 구간에서 `!enabled`가 참이 되어, 이미 빛나고 있는 에디터 위에 `Enable`을 내민다. 렌더러는
누구를 기다리지 않고 `localStorage`에서 자기 상태를 복원하기 때문이다. 키가 둘이면 그 구간이
*아직 모름*으로 읽힌다. 어느 것도 목록에 없고, context key가 아니라 상태 파일을 읽는 `Toggle`은
그 사이에도 동작한다.
...
생겼는데 아무 일도 안 일어나는 식이다. 주입된 배너가 페이로드의 해시를 함께 담고 있어서, 확장이
불일치를 알아채고 다시 패치할지 물어본다. 수락하면 다른 패치와 마찬가지로 완전 재시작이 필요하다.

버전 번호가 아니라 페이로드의 해시인 것은 의도적이다. 대부분의 릴리스는 확장만 바꾸는데, 버전을
찍어두면 이미 바이트 단위로 올바른 번들에 대해서까지 재패치와 재시작을 요구하게 된다.
