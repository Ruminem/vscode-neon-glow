...
같은 파일의 같은 순간임. 왼쪽은 글로우 없음, 오른쪽은 글로우 있음 — 토큰 색, 괄호 짝
상자, 커서가 놓인 심볼의 다른 출현, 에러·경고·정보 물결선까지:

![같은 에디터를 좌우로 놓은 그림. 왼쪽은 평범한 구문 강조이고, 오른쪽은 같은 줄들이 테마가 준 색 그대로 빛남. 커서 옆 괄호 짝 상자가 밝고, 커서가 놓인 심볼의 다른 출현이 드러나며, 물결선 셋이 테마의 진단 색을 띰](https://raw.githubusercontent.com/Ruminem/vscode-neon-glow/main/images/neon-glow.png)
