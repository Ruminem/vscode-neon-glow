...
| 명령 | |
|------|--|
| `Neon Glow: 설정/해제` | 글로우를 즉시 켜고 끔 |
| `Neon Glow: 사용` / `Neon Glow: 사용 안 함` | 명시적으로 지정. 실제로 뭔가 바뀌는 쪽만 목록에 뜸 |
| `Neon Glow: 상태 표시` | 지금 상태와 번들이 패치돼 있는지 |
| `Neon Glow: 프리셋 선택` | 은은하게 / 기본 / 다 켜기 — 노브 스물셋을 세 답으로. 재시작 없이 즉시 적용됨 |
...
`Neon Glow: 상태 표시`를 불러도 됨. 어느 쪽이든 쓰기 권한과 **완전 재시작**이 필요함.
...
만들고 특이도가 같아서, 나중에 붙은 쪽이 이김. `Neon Glow: 상태 표시`가 지금 경합
중인지 알려주고, `Neon Glow: 사용 안 함`은 파일을 건드리지 않고 즉시 비켜줌.
